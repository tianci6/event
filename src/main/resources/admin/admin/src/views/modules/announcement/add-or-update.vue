<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <!-- <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="ID" prop="annoucementid">
          <el-input v-model="ruleForm.annoucementid" 
              placeholder="ID" clearable  :readonly="ro.annoucementid"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="ID" prop="annoucementid">
              <el-input v-model="ruleForm.annoucementid" 
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.announcementphoto" label="Photo" prop="announcementphoto">
          ...
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.announcementphoto?ruleForm.announcementphoto:''"
          @change="juminzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.announcementphoto" label="Photo" prop="announcementphoto">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.announcementphoto.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  data() {
    ...
    return {
      ...
      type: '',
      ro:{
	announcementlatitude : false,
	announcemnetlongitude : false,
	announcementdescription : false,
	announcementphoto : false,
      },
      ruleForm: {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.nianling = true;
          ...
          if(o=='juminzhaopian'){
            this.ruleForm.announcementphoto = obj[o];
	    this.ro.announcementphoto = true;
            continue;
          }
          if(o=='juminshouji'){
	    ...
	    this.ro.juminshouji = true;
          ...
          if(o=='juzhudizhi'){
            this.ruleForm.juzhudizhi = obj[o];
	    this.ro.juzhudizhi = true;
            continue;
          }
          if(o=='menpaihao'){
	...
	if(this.ruleForm.announcementphoto!=null) {
		this.ruleForm.announcementphoto = this.ruleForm.announcementphoto.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `annoucement/${!this.ruleForm.annoucementid ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
    ...
    juminzhaopianUploadChange(fileUrls) {
	this.ruleForm.announcementphoto = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
